...
if file:
    ...
    with tab1:
        ...
        fig = go.Figure()
        x_axis = df.index if date_col else df.index
        fig.add_trace(go.Scatter(x=x_axis, y=df[close_col], mode='lines', name='Close', line=dict(color='blue')))
        fig.add_trace(go.Scatter(x=x_axis, y=df['MA30'], mode='lines', name='MA30', line=dict(color='orange', width=2)))
        fig.add_trace(go.Scatter(x=x_axis, y=df['MA90'], mode='lines', name='MA90', line=dict(color='green', width=2)))
        ...
        with col1:
            ...
            fig_volat = go.Figure()
            fig_volat.add_trace(go.Scatter(x=x_axis, y=df['Volatility'], mode='lines', line=dict(color='red')))
        ...
        with col2:
            ...
            fig_macd.add_trace(go.Scatter(x=macd_val.index, y=macd_val, fill='tozeroy', mode='lines', line=dict(color='green')))
    ...
    with tab2:
        ...
        if len(df_ml) > 100:
            ...
            fig_ml = go.Figure()
            x_test_axis = df_ml.index[-len(y_test):] if date_col else df_ml.index[-len(y_test):]
            fig_ml.add_trace(go.Scatter(x=x_test_axis, y=y_test, mode='lines', name='Actual Price'))
            fig_ml.add_trace(go.Scatter(x=x_test_axis, y=pred, mode='lines', name='Predicted Price', line=dict(dash='dash', color='red')))
            fig_ml.update_layout(hovermode="x unified", height=400)
            ...
            if st.button("▶️ Run Deep Learning LSTM Forecast"):
                with st.spinner("Training LSTM Network (may take a minute)..."):
                    try:
                        ...
                        fig_lstm.update_layout(hovermode="x unified", height=400)
    ...
    with tab3:
        ...
        fig_sig.add_trace(go.Scatter(x=df.index, y=df[close_col], mode='lines', name='Close Price', line=dict(color='black', width=1)))
        fig_sig.add_trace(go.Scatter(x=df.index, y=df['MA7'], mode='lines', name='MA7', line=dict(color='blue', width=1, dash='dot')))
        fig_sig.add_trace(go.Scatter(x=df.index, y=df['MA30'], mode='lines', name='MA30', line=dict(color='orange', width=1, dash='dot')))
        
        buy_points = df[df['Buy_Signal']]
        sell_points = df[df['Sell_Signal']]
        
        fig_sig.add_trace(go.Scatter(x=buy_points.index, y=buy_points[close_col], mode='markers', name='Buy Signal', marker=dict(color='green', symbol='triangle-up', size=12)))
        fig_sig.add_trace(go.Scatter(x=sell_points.index, y=sell_points[close_col], mode='markers', name='Sell Signal', marker=dict(color='red', symbol='triangle-down', size=12)))
        ...
        fig_port = go.Figure()
        fig_port.add_trace(go.Scatter(x=df.index, y=df['Portfolio_Value'], mode='lines', line=dict(color='darkgreen')))
    ...
    with tab4:
        ...
        fig_dd = go.Figure()
        fig_dd.add_trace(go.Scatter(x=x_axis, y=drawdown, fill='tozeroy', mode='lines', line=dict(color='red')))
    ...
    with tab5:
        ...
        for col in numeric_cols:
            ...
            with col_a:
                fig_line = go.Figure()
                fig_line.add_trace(go.Scatter(x=df.index if date_col else df.index, y=df[col], mode='lines', line=dict(color='teal')))
